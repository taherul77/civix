...
export const useData = create<DataState>((set, get) => ({
  // Local store starts empty — every list is sourced from the backend via
  // the api.* / queries.* hooks. The remaining keys below (audit log,
  // equipment readings, ZATCA CSID) stay because they are local-only state
  // that the backend doesn't model yet.
  projects: [],
  samples: [],
  tests: [],
  equipment: [],

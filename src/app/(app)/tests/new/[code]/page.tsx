...
export default function GenericTestForm({ params }: { params: Promise<{ code: string }> }) {
  ...
  const computedResults = useMemo(() => schema.results.map((r) => {
    const v = r.compute(values);
    if (typeof v === "number") return { ...r, value: v.toFixed(r.decimals ?? 2) };
    return { ...r, value: v };
  }), [schema, values]);

  const passes = schema.passRules.map((r) => ({ label: r.label, ok: r.ok(values) }));
  const overall = passes.length === 0 ? "pending" : passes.every((p) => p.ok) ? "pass" : "fail";
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <button className="btn btn-primary"><Send className="w-4 h-4" /> Submit</button>
          </div>
        }
      />
